...
#include "AC_LADRC.h"                  // Model header file

static AC_LADRC AC_LADRC_Obj;          // Instance of model class
...
void rt_OneStep(void)
{
  ...
  // Check for overrun
  if (OverrunFlag) {
    rtmSetErrorStatus(AC_LADRC_Obj.getRTM(), "Overrun");
    return;
  }
  ...
  // Step the model
  AC_LADRC_Obj.step(arg_attitude_error, arg_rate_ff, arg_rate_meas, arg_Output);
...
int_T main(int_T argc, const char *argv[])
{
  ...
  fflush((nullptr));
  while (rtmGetErrorStatus(AC_LADRC_Obj.getRTM()) == (nullptr)) {
    //  Perform application tasks here
  }

  // Terminate model
  AC_LADRC_Obj.terminate();
  return 0;
}
